...
description: 将中文剧本、小说片段或网页内容转换为适合 AI 短片图片生成的客观分镜头脚本。用于用户要求“剧本转分镜”“按片段生成分镜表”“AI短片分镜脚本”“输出Markdown或Excel分镜表”，输入可为 .md、.txt、.docx、.pdf、.html 文件或网页 URL，并需要严格按照剧本已有内容输出镜号、时长、景别、运镜、画面内容提示词、人物台词、备注，同时审查画面合理性、时长合理性、对话覆盖和运镜可实现性。
---
...
3. 使用两阶段模型流程：
   - 初稿：DeepSeek V4 Pro，负责按剧本事实拆分镜头，生成客观分镜表。
   - 审查：Claude Sonnet 4.6，以经验导演编剧身份检查时长、画面、台词覆盖和运镜可实现性。
4. 检查输出是否满足每个镜头至少包含：镜号、时长、景别、运镜、画面内容提示词、人物台词、备注。
5. 若用户要继续生成图片，把 `画面内容提示词` 列作为后续图像生成输入，不要把对白、字幕、镜号或不可见心理活动塞进画面提示词。
6. 严格按照剧本已有内容客观编写，不添油加醋，不新增剧情、人物、道具、地点或华丽修饰。
...
如果脚本当前只导出分镜 Markdown/Excel，则在最终回复中说明 `画面内容提示词` 列已经可作为提示词包使用；只有用户要求单独整理提示词包时，再创建 `07_绘图提示词` 文件。
...
- 全部中文。
- 表格字段使用：镜号、时长、景别、运镜、画面内容提示词、人物台词、备注。
- 描述要直观、客观、可生图，避免华丽辞藻。
- 严格按剧本已有内容写，不新增剧情、人物、道具、地点或情绪。
- 景别使用常见词：远景、全景、中景、近景、特写、大特写、俯拍、仰拍、过肩镜头。
- 画面内容提示词写清主体、动作、场景、时间、可见道具、光线、构图、画幅，适合生成单张分镜图片。
- 人物台词只写剧本已有台词；没有台词留空。
- 时长要合理：普通画面 2-4 秒，动作或转场 1-3 秒，较长台词镜头 4-6 秒；长台词要拆镜头。
- 运镜要可实现：固定镜头、轻微推进、轻微后拉、平移、跟拍、切到特写优先；避免复杂调度、快速旋转或连续多机位。
- 每个镜头备注中可写审查说明，例如“台词时长匹配”“单张图片可生成”“动作过多已拆分”。
