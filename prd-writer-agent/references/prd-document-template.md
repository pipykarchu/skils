...
> 适用于用 AI 参与开发的项目。统计开发阶段各平台的 token 用量和金额，包括 Codex、Claude 以及其他实际使用的平台，并为后期计算 ROI 保留收益口径。不要编造数据；缺失时标记为 `待统计` 或 `待确认`。
...
当用户更新开发工具、模型供应商、模型路由、API 地址、角色分工、运行链路、里程碑、环境变量、隐私、安全策略、token 用量、开发成本、收益假设或 ROI 口径时，同步更新本节，并在修订记录中说明原因。
