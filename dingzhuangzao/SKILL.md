...
- Key actions (❤️ / 删除 / 双击预览 / 空卡双击生成 / 图生图空卡上传 / 图生图空卡拖拽上传 / 进入下一版 / 确认此造型 / 导入 / 生成意图) **auto-save** to `selection-state.json`; the `保存选择` button is a manual fallback. Double-click preview does not modify state; empty-card double-click can consume API balance after confirmation; 图生图空卡上传/拖拽上传 only copies the selected local image into project assets and updates `manifest.json`. Clicking `进入下一版` must immediately render a visible status badge on the relevant cards, e.g. `下一版待生成`.
- 验收标准：在统一评审页中，任意 `图生图` 空候选卡都必须能直接拖入本地 PNG/JPG/WebP 并完成上传；如果只能点击上传、只能用顶部导入、或拖入后没有写回该卡槽位，则该页不合格，必须修复后再交付。
...
## 2026-06 visual review addendum

Use these rules for unified visual-review pages copied from recent manju projects:

- `进入下一版` must show which model will generate the next round, either as a visible selector beside the button or a clear hover/title text on the control. Do not silently hard-code Gemini Image. Persist the chosen engine as `nextEngine` in `selection-state.json`, and write it into `nextRoundRequests[]`.
- Character next rounds keep `outputType = "character_turnaround_3view"` regardless of engine. Scene/prop next rounds keep `outputType = "visual_candidate"` and normally use horizontal 16:9 unless the project says otherwise.
- Empty Image2 / Nano Banana Pro / Gemini-style API cards may call `/api/generate-slot` and must write the result back into the exact manifest slot so the image appears in the page after refresh.
- Midjourney is allowed to remain an external/manual channel. If the page only opens MJ and copies the prompt, clearly label it as manual. For MJ images to be visible in the review page, the generated file must later be imported or uploaded back into the exact MJ slot and written into `manifest.json`; prompt-launch records alone are not enough.
- For stories where looking across a physical barrier is part of the theme, encode the barrier as a hard spatial anchor in the relevant state. Example: `顾明昭的临湖小窗在三楼，窗外先隔顾府高墙，再隔湖水与画舫相望`. This belongs in worldview `space`, generation prompts, and next-round adjustment notes, not only in chat history.
