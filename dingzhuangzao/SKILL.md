...
- Key actions (❤️ / 删除 / 双击预览 / 空卡双击生成 / 进入下一版 / 确认此造型 / 导入 / 生成意图) **auto-save** to `selection-state.json`; the `保存选择` button is a manual fallback. Double-click preview does not modify state; empty-card double-click can consume API balance after confirmation.
