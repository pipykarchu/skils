...
- Empty Gemini/Image2/MJ rows are clickable to record a **generate intent** (`STATE.gen`, persisted as `genRequests`); for 人物定妆, `genRequests[].outputType` must be `character_turnaround_3view`. The server has no image API, so this only records intent for the agent to act on
...
- Key actions (❤️ / 删除 / 双击预览 / 进入下一版 / 确认此造型 / 导入 / 生成意图) **auto-save** to `selection-state.json`; the `保存选择` button is a manual fallback. Double-click preview does not modify state.
